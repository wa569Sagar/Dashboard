...
st.set_page_config(
    page_title="EXCO Monthly Reporting ECAG",
    page_icon="📊",
    layout="wide",
    menu_items={
...
LOGO_FILE = Path(__file__).resolve().parent / "assets" / "eurex_logo.png"
...
SEVERITY_COLORS = {
    "S4": "#9B59B6",  # Purple (critical)
    "S3": "#E74C3C",  # Red (high)
    "S2": "#E67E22",  # Orange (medium)
    "S1": "#F1C40F",  # Yellow (low)
}
...
def get_data_snapshot_date() -> str:
    """Get the snapshot date based on when the data file was last modified."""
    if DATA_FILE.exists():
        mod_time = DATA_FILE.stat().st_mtime
        return datetime.fromtimestamp(mod_time).strftime("%d %b %Y")
...
def render_header(df: pd.DataFrame, selected_sheet: str, is_sii: bool = False) -> None:
    ...
    # Build HTML string without f-string to avoid issues
